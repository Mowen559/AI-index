...
# Replace the AI tab content block
pattern = r'\{/\* INFO TAB CONTENT \*/\}.*?\{/\* GIT HISTORY TAB CONTENT \*/\}'
            ...
            {!error && data && (
              <div className={`space-y-6 transition-opacity duration-200 ${loading ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
                ...
                <div className="flex justify-between items-center">
                  <h2 className="text-lg font-bold text-white font-serif tracking-wide">{(data.file || selectedFile).split('/').pop()}</h2>
...
content = re.sub(pattern, new_info_tab, content, flags=re.DOTALL)
